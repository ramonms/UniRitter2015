...
namespace UniRitter.UniRitter2015.Specs
{
    ...
    public class PeopleAPISteps
    {
        ...
        public void ThenIGetAListContainingThePopulatedResources(String resource)
        {
            ...
            {
                case "people":
                    var rListPeople = response.Content.ReadAsAsync<IEnumerable<Person>>().Result;
                    Assert.That(backgroundData, Is.SubsetOf(rListPeople));
                    break;
                case "posts":
                    var rListPost = response.Content.ReadAsAsync<IEnumerable<Post>>().Result;
                    Assert.That(backgroundDataPost, Is.SubsetOf(rListPost));
                    break;
            }
        ...
        public void ThenIGetTheRecordThatMatchesThatId(String tp)
        {
            ...
            switch (tp)
            {
                case "people":
                    result = response.Content.ReadAsAsync<Person>().Result;
                    var expec = backgroundData.Single(p => p.id == id);
                    Assert.That(result, Is.EqualTo(expec));                    
                    break;
                case "posts":

                    resultPost = response.Content.ReadAsAsync<Post>().Result;
                    var expecPost = backgroundDataPost.Single(p => p.id == id);
                    Assert.That(resultPost, Is.EqualTo(expecPost));
                    break;
            }
